<x-app-layout>
    ...
    <form action="{{ route('admin.branches.update', $branch) }}" method="POST">
        @csrf
        @method('PUT')
        {{-- Fieldset akan menonaktifkan semua input di dalamnya jika user adalah Owner --}}
        <fieldset @if(Auth::user()->isOwner()) disabled @endif>
            <div class="space-y-4">
                <div>
                ...
                <div class="flex justify-end">
                    <a href="{{ route('admin.branches.index') }}" class="px-4 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400 mr-2">
                    ...
                    {{-- Tombol Update hanya muncul jika pengguna BUKAN Owner --}}
                    @if(!Auth::user()->isOwner())
